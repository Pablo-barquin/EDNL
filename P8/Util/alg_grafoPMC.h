...
#include "particion.cpp"      // para Kruskall
                        ...
                        vector<typename GrafoP<tCoste>::vertice>& P)
{
   ...
   const size_t n = G.numVert();
   vector<bool> S(n, false);                  // Conjunto de vértices vacío.
   vector<tCoste> D;                          // Costes mínimos hacia destino.

   // Iniciar D y P con caminos directos hacia el vertice destino.
   for(v = 0; v < n; ++v){
      D.push_back(G[v][destino]);
   }                               
   D[destino] = 0;    // Coste destino-destino es 0.
   P = vector<vertice>(n, destino);

   // Calcular caminos de coste mínimo hacia el destino.
   S[destino] = true;                          // Incluir vértice destino en S.
   for (size_t i = 1; i < n-1; i++) {
      // Seleccionar vértice w no incluido en S
      // con menor coste hacia origen.
      tCoste costeMin = GrafoP<tCoste>::INFINITO;
      for (v = 0; v < n; v++)
         if (!S[v] && D[v] <= costeMin) {
            costeMin = D[v];
